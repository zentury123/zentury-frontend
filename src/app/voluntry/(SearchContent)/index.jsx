...
function SearchContent() {
  ...
  const { data, loading } = useGetRealState("auction", priceRange, selectedRegion, propertyType, auctionType, pageNumber,keywords);
  ...
  const debouncedSetKeywords = useDebounce((newKeyword)=>{setKeywords(newKeyword)}, 500);

  const deboucePriceRange = useDebounce((price)=>{setPriceRange(price)}, 500);
    ...
    <div>
        ...
        <div className="grid xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1  mt-[31px] gap-6">
          {data.map((item) => (
            ...
            >
              <div className="relative">
                <img src={item?.mainImage} alt="" className="object-cover w-full h-[280px]" />
